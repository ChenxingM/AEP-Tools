...
写入属性值时，`aep_tools` 会自动处理 AE 二进制格式的若干约束：

- **Anchor Point 始终 3 分量** — AE 内部始终以 `[x, y, z]` 存储锚点，即使 2D 图层 z 也固定为 0。写入 2 分量值如 `[50, 50]` 时会自动补齐为 `[50, 50, 0]`。
- **Position 取决于图层状态** — 2D 图层为 `[x, y]`，3D 图层为 `[x, y, z]`。3D 图层上写入 2 分量值会自动补齐。
- **默认属性自动创建** — 某些 Transform 属性（锚点、位置等）在默认值时不存在于二进制文件中。写入时会自动创建所需的二进制结构（tdbs/tdb4/cdat）。
- **维度自动升级** — 如果二进制中的属性分量数少于写入值的分量数，元数据（tdb4）和数据（cdat）会自动重建以匹配。
...
| 值 | 名称 | 值 | 名称 |
|---|---|---|---|
| 2 | NORMAL | 21 | LUMINESCENT_PREMUL |
| 3 | DISSOLVE | 22 | ALPHA_ADD |
| 4 | ADD | 23 | CLASSIC_COLOR_DODGE |
| 5 | MULTIPLY | 24 | CLASSIC_COLOR_BURN |
| 6 | SCREEN | 25 | EXCLUSION |
| 7 | OVERLAY | 26 | DIFFERENCE |
| 8 | SOFT_LIGHT | 27 | COLOR_DODGE |
| 9 | HARD_LIGHT | 28 | COLOR_BURN |
| 10 | DARKEN | 29 | LINEAR_DODGE |
| 11 | LIGHTEN | 30 | LINEAR_BURN |
| 12 | CLASSIC_DIFFERENCE | 31 | LINEAR_LIGHT |
| 13 | HUE | 32 | VIVID_LIGHT |
| 14 | SATURATION | 33 | PIN_LIGHT |
| 15 | COLOR | 34 | HARD_MIX |
| 16 | LUMINOSITY | 35 | LIGHTER_COLOR |
| 17 | STENCIL_ALPHA | 36 | DARKER_COLOR |
| 18 | STENCIL_LUMA | 37 | SUBTRACT |
| 19 | SILHOUETTE_ALPHA | 38 | DIVIDE |
| 20 | SILHOUETTE_LUMA | | |
